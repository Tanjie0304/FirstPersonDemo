...
protected:
	...
	// NPC 是否处于“游戏未开始”的锁定状态
	UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category = "Shooter|PreGame")
	bool bAIInputLocked = false;

	// 记录正常的移动速度，解锁时恢复
	float DefaultWalkSpeed = 0.0f;

	// 监听 GameState 预备倒计时的回调
	UFUNCTION()
